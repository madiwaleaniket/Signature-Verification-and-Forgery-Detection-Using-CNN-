...
def extract_sign(doc):
    ...
    img = cv2.imdecode(np.fromstring(doc.read(), np.uint8), cv2.IMREAD_COLOR)
    ...
    img_data = pre_version.astype(np.uint8)
    _, img_str = cv2.imencode('.png', img_data)
    # Decode image data using cv2.imdecode
